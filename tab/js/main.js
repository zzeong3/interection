...
const speed = converSpeed(boxes[0]); // transition은 각 요소마다 있으니까
let enableClick = true; // 처음은 클릭 모두가 클릭되어야 하니가
...
btns.forEach((btn, index)=>{
    ...
    btn.addEventListener('click', (e)=>{
        ...
        //on이 있니? contains는 blooean 값으로 뱉어줌
        if(isOn) return;

        //클릭한 순간!!
        if(enableClick) { // 클릭해서 참으로 들어왔지만
            enableClick = false; //클릭한 순간 다시 false, if문에 막혀서 다른 버튼 클릭안돼!
            // 함수 호춯
...
function converSpeed(el) { // transition은 각 요소마다 있으니까
